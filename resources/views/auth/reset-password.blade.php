<x-guest-layout>
    <div class="w-full max-w-sm p-4 bg-bgprimary border-bgprimary rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
        ...
        <form method="POST" action="{{ route('password.store') }}">
            @csrf
            <h5 class="text-xl font-medium text-textprimary dark:text-white">Reset Password</h5>
            <!-- Password Reset Token -->
            <input type="hidden" name="token" value="{{ $request->route('token') }}">

            <!-- Email Address -->
            <div>
                <x-input-label for="email" class="text-textprimary" :value="__('Email')" />
            ...
            <!-- Password -->
            <div class="mt-4">
                <x-input-label for="password" class="text-textprimary" :value="__('Password')" />
            ...
            <!-- Confirm Password -->
            <div class="mt-4">
                <x-input-label for="password_confirmation" class="text-textprimary" :value="__('Confirm Password')" />
